...
export default function RecycleItem({ bookID, books, setBooks }){
    ...
    function remove(){
        ...
        localStorage.setItem("neoword-recycle", JSON.stringify(newBooks))
        localStorage.removeItem(`neoword-item-${bookID}`)
        setBooks(newBooks)
    }

    function restore(){
        const allBooks = readLocal("neoword-books")
        allBooks.push(bookID)
        localStorage.setItem("neoword-books", JSON.stringify(allBooks))
